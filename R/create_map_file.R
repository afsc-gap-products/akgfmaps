...
create_map_file <- function(x,
  ...
  if(try.change_text_size) {
    if(width == 12 & height == 9 & units == "in") {in.dat <- change_text_size(x = in.dat, size.mult = 1.2); size.changed <- TRUE}
    if(width == 8 & height == 6 & units == "in") {in.dat <- change_text_size(x = in.dat, size.mult = 0.8); size.changed <- TRUE}
  } else {
    size.changed <- TRUE
  }
